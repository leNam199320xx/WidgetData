...
Hiện tại **WidgetData.AppHost** orchestration 4 service chính (`API`, `Worker`, `Gateway`, `Web`).  
Các demo front-end trong thư mục `demo/` chạy độc lập bằng static server và gọi API live.

Ngoài các demo front-end, repo có bộ SQL data test tại `scripts/sql/` gồm `sales-test.sql`, `course-test.sql`, `news-test.sql` và bộ mới `retail-test.sql` để test tenant retail (inventory/sales/procurement).
...
Khởi động backend qua AppHost:

```bash
dotnet run --project src/WidgetData.AppHost
```

Sau đó chạy từng demo front-end theo hướng dẫn ở trên (hoặc xem chi tiết trong `doc/demo-guide.md`).
